// src/app/api/cron/tipster-concours-week/route.ts
// Cron hebdo : calcul du gagnant de la semaine pr\u00e9c\u00e9dente
// Appell\u00e9 chaque lundi \u00e0 00h15 par Vercel
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!config.week.active) {
      return NextResponse.json({
        skipped: true,
        reason: "Concours semaine d\u00e9sactiv\u00e9",
        period: periodStart,
      });
    }
    const minPicks = config.week.min_picks;
    const prize = config.week.prize_amount;
    ...
    // R\u00e9cup\u00e9rer tous les picks r\u00e9solus de la p\u00e9riode
    ...
    if (resend && inserted) {
      ...
      if (user?.email) {
        const weekStartStr = bounds.start.toLocaleDateString("fr-FR", { day: "numeric", month: "long" });
        const weekEndStr = bounds.end.toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" });
        ...
        try {
          await resend.emails.send({
            ...
            subject: "\ud83c\udfc6 F\u00e9licitations ! Tu as gagn\u00e9 le concours de la semaine",
            html: `
              <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: linear-gradient(135deg, #0a0a0a 0%, #062e1f 50%, #0a0a0a 100%); color: white; padding: 40px 24px; border-radius: 16px;">
                <div style="text-align: center; margin-bottom: 32px;">
                  ...
                  <h1 style="font-size: 24px; font-weight: 800; margin: 0; color: #34d399;">F\u00e9licitations ${user.pseudo || ""} !</h1>
                  <p style="font-size: 14px; color: rgba(255,255,255,0.7); margin-top: 8px;">Tu es le meilleur tipster de la semaine \u00e0 PRONOS.CLUB</p>
                </div>

                <div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(16,185,129,0.3); border-radius: 12px; padding: 24px; margin-bottom: 24px; text-align: center;">
                  <p style="font-size: 11px; font-weight: 700; letter-spacing: 0.2em; text-transform: uppercase; color: rgba(255,255,255,0.5); margin: 0 0 8px;">Semaine du</p>
                  <p style="font-size: 16px; font-weight: 700; color: white; margin: 0 0 20px;">${weekStartStr} au ${weekEndStr}</p>

                  <div style="display: inline-block; background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.4); border-radius: 12px; padding: 16px 32px;">
                    <p style="font-size: 11px; font-weight: 700; letter-spacing: 0.2em; text-transform: uppercase; color: #6ee7b7; margin: 0 0 4px;">Ton gain</p>
                    <p style="font-size: 36px; font-weight: 900; color: #34d399; margin: 0;">${prize} \u20ac</p>
                  </div>

                  <div style="display: flex; justify-content: space-around; margin-top: 24px; padding-top: 20px; border-top: 1px dashed rgba(255,255,255,0.1);">
                    <div>
                      ...
                      <p style="font-size: 18px; font-weight: 800; color: #34d399; margin: 4px 0 0;">+${Math.round(winner.total_units * 100) / 100}</p>
                    </div>
                    <div>
                      ...
                      <p style="font-size: 18px; font-weight: 800; color: white; margin: 4px 0 0;">${winner.total_picks}</p>
                    </div>
                  </div>
                </div>
                ...
                <div style="text-align: center;">
                  <a href="https://pronos.club/fr/pronos-abonnes/concours" style="display: inline-block; background: #10b981; color: white; padding: 14px 32px; border-radius: 12px; text-decoration: none; font-weight: 700; font-size: 14px;">Voir le concours</a>
                </div>

                <p style="text-align: center; font-size: 11px; color: rgba(255,255,255,0.3); margin-top: 32px; letter-spacing: 0.1em;">PRONOS.CLUB \u00b7 Continue comme \u00e7a \ud83d\ude80</p>
              </div>
            `,
          });
        } catch (emailErr) {
          console.error("[cron week] email error:", emailErr);
        }
      }
    }
    ...
    console.error("[cron week] error:", err.message);
