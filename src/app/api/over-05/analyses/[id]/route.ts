...
// → Retourne l'état d'une analyse + ses résultats partiels au fur et à mesure
//   que le job background avance.
//
// Utilisé en polling toutes les 3s par le frontend pendant qu'une analyse
// est "running".
...
export async function GET(
  ...
  // Récupérer la session d'analyse
  ...
  // Sécurité supplémentaire : seul le user qui a créé l'analyse peut la voir
  if (analysis.requested_by !== user.email) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  // Récupérer les match analyses déjà calculés
