...
interface StatsData {
  ...
  aiBankroll: { mode: string; initial_bankroll: number; current_bankroll: number; unit_value: number; unit_percent: number; auto_recalc: string; show_on_site: boolean } | null;
}
      ...
      <div className="mx-auto max-w-4xl px-4 pt-6 text-center">
        {showEuro && bk && uv > 0 && (
          <div className="mb-4 inline-flex items-center gap-4 rounded-full border border-violet-500/20 bg-violet-500/5 px-5 py-2">
            <span className="text-xs text-neutral-500">Bankroll : <span className="font-bold text-neutral-900">{bk.current_bankroll.toLocaleString("fr-FR")}€</span></span>
            <span className="text-xs text-neutral-500">1U = <span className="font-bold text-violet-600">{uv.toFixed(2)}€</span></span>
          </div>
        )}
            ...
            <div className="mb-8 mt-4 grid gap-3 sm:grid-cols-2">
              {o.bestPick && (
                <div className="overflow-hidden rounded-2xl border-l-4 border-emerald-500 p-5 text-center" style={{ background: "linear-gradient(135deg, #0a0a0a 0%, #062e1f 100%)" }}>
                  ...
                  <p className="mt-1 font-bold text-white">{o.bestPick.pickNumber ? `#${o.bestPick.pickNumber} ` : ""}{o.bestPick.event}</p>
              ...
              {o.worstPick && (
                <div className="overflow-hidden rounded-2xl border-l-4 border-red-500 p-5 text-center" style={{ background: "linear-gradient(135deg, #0a0a0a 0%, #2e0606 100%)" }}>
                  ...
                  <p className="mt-1 font-bold text-white">{o.worstPick.pickNumber ? `#${o.worstPick.pickNumber} ` : ""}{o.worstPick.event}</p>
