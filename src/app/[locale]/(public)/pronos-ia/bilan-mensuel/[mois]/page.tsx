/**
 * ═══════════════════════════════════════════════════════════════════
 * /pronos-ia/bilan-mensuel/[mois]/page.tsx
 * ═══════════════════════════════════════════════════════════════════
 *
 * Page bilan mensuel d'un mois calendaire donné.
 *
 * URL : https://pronos.club/[locale]/pronos-ia/bilan-mensuel/[mois]
 * Slug : "mois-mai-2026" (cohérent avec "semaine-19-2026" pour hebdo)
 *
 * Lit les données depuis monthly_bilans (cache préagrégé par le cron
 * 1er du mois 22h Paris). Si le mois n'existe pas → 404.
 *
 * Layout (réplique du pattern bilan-hebdo) :
 *   - Hero header (mois + KPIs ROI + profit)
 ...
 *   - Disclaimer ANJ
 *
 * Path : src/app/[locale]/(public)/pronos-ia/bilan-mensuel/[mois]/page.tsx
 * ═══════════════════════════════════════════════════════════════════
 */
...
import { getMonthlyBilanBySlug } from "@/lib/bilan/mensuel-generator";
import { BilanMensuelCharts } from "@/components/bilan/BilanMensuelCharts";

// IMPORTANT Next.js 16 : params est une Promise
type PageProps = {
  params: Promise<{ locale: string; mois: string }>;
};
...
export async function generateMetadata(
  ...
  const { mois, locale } = await props.params;
  const bilan = await getMonthlyBilanBySlug(mois);

  if (!bilan) {
    return {
      title: "Bilan mensuel introuvable — PRONOS.CLUB",
    };
  }

  const roiSign = bilan.roi_pct >= 0 ? "+" : "";
  const title = `Bilan ${bilan.month_label} : ROI ${roiSign}${bilan.roi_pct.toFixed(2)}% sur ${bilan.total_picks} picks IA | PRONOS.CLUB`;
  const desc = `Bilan complet du mois ${bilan.month_label} : ${bilan.picks_won} gagnés / ${bilan.picks_lost} perdus, profit ${bilan.total_profit_units >= 0 ? "+" : ""}${bilan.total_profit_units.toFixed(2)}U, CLV moyen ${bilan.clv_avg_pct !== null ? (bilan.clv_avg_pct >= 0 ? "+" : "") + bilan.clv_avg_pct.toFixed(2) + "%" : "n/a"}. Pronostics IA gratuits, transparence totale.`;
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `/api/og/bilan-mensuel/${mois}`,
          width: 1200,
          height: 630,
          alt: `Bilan ${bilan.month_label} — PRONOS.CLUB`,
        },
      ],
    },
    twitter: {
      ...
      title,
      description: desc,
      images: [`/api/og/bilan-mensuel/${mois}`],
    },
    alternates: {
      canonical: `/${locale}/pronos-ia/bilan-mensuel/${mois}`,
    },
  };
}
...
export default async function BilanMensuelPage(props: PageProps) {
  const { mois, locale } = await props.params;
  const bilan = await getMonthlyBilanBySlug(mois);
      ...
      <div className="container mx-auto max-w-5xl px-4 py-8 md:py-12">
        ...
        <header className="mb-10 border-b border-zinc-200 pb-8">
          ...
          <p className="text-lg text-zinc-600 mb-6">
            Bilan complet de la performance IA sur le mois
          </p>
