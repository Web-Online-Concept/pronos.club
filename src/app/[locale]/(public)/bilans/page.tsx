...
export default async function BilansPage({ params }: { params: Promise<{ locale: string }> }) {
          ...
          {items.length > 0 && (
            <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5">
              <span className="text-xs font-semibold text-emerald-400">
                {items.length > 1 ? t("badge_many", { count: items.length }) : t("badge_one", { count: items.length })}
              </span>
            </div>
          )}
        </div>
