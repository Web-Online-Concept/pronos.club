...
const MODE_OPTIONS: { value: BkMode; label: string; desc: string; icon: string; accent: string }[] = [
  ...
  },
  {
    ...
    label: "% de bankroll",
    desc: "Votre mise s'adapte automatiquement. Quand vous gagnez, la mise monte. Quand vous perdez, elle descend.",
    icon: "📈",
    accent: "#10b981",
  },
];
...
export default function GestionBKPage() {
  ...
  // When switching mode, reset irrelevant fields
  ...
  const validate = useCallback((): string | null => {
    ...
    if (!bk || bk <= 0) return "Veuillez renseigner votre bankroll initiale.";

    if (mode === "fixed_unit") {
      const uv = parseFloat(unitValue);
      if (!uv || uv <= 0) return "Veuillez renseigner la valeur de 1 unité.";
      if (uv > bk) return "La valeur de l'unité ne peut pas dépasser votre bankroll.";
    }

    if (mode === "percent_bankroll") {
      const up = parseFloat(unitPercent);
      if (!up || up <= 0) return "Veuillez renseigner le pourcentage par unité.";
      if (up > 25) return "Un pourcentage supérieur à 25% est très risqué. Êtes-vous sûr ?";
    }

    return null;
  }, [mode, bankroll, unitValue, unitPercent]);

  async function handleSave() {
    const validationError = validate();
    if (validationError && !validationError.includes("Êtes-vous sûr")) {
      setError(validationError);
      return;
    }
    // For warnings (like >25%), allow save but show warning
    if (validationError) {
      setError(validationError);
      // Still save — it's a warning, not a blocker
    }
    ...
    // For fixed_unit: bk / unit_value
    // For percent_bankroll: 100 / percent (e.g. 1% = 100 units)
  ...
  const inputClass =
    "w-full rounded-xl border border-white/10 bg-white/[0.05] px-4 py-3 text-center font-mono text-lg font-bold text-white outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 [color-scheme:dark] placeholder-white/20";

  if (loading) {
    return (
      <>
        <EspaceHero title="Ma Gestion de BK" />
    ...
    <>
      ...
      <main className="mx-auto max-w-lg px-4 pb-12 pt-6">
        <p className="text-center text-sm text-neutral-400">
          Personnalisez votre gestion de bankroll
        </p>

        {/* Current bankroll card — only if already configured with monetary mode */}
        {config.mode !== "units_only" && config.current_bankroll > 0 && (
          ...
          >
            <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-emerald-400/60">Bankroll actuelle</p>
        ...
        {mode !== "units_only" && (
          <div className="mt-6 space-y-5">
            ...
            >
              ...
              <p className="mb-2 text-[10px] text-white/15">Le montant total que vous investissez au départ.</p>
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setError("");
                    // Si la BK actuelle n'a jamais été modifiée, on la sync
            ...
            >
              ...
              <p className="mb-2 text-[10px] text-white/15">Votre capital actuel. Modifiez-le à tout moment si besoin.</p>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  step="0.01"
            ...
            {mode === "fixed_unit" && (
              ...
              >
                ...
                <p className="mb-2 text-[10px] text-white/15">Combien vaut 1U en euros. Ex: BK de 1000€ → 1U = 10€ (1% de la BK).</p>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    step="0.01"
            ...
            {mode === "percent_bankroll" && (
              ...
              >
                ...
                <p className="mb-2 text-[10px] text-white/15">Conseil : entre 1% et 3% pour une gestion saine. Au-delà de 5%, le risque augmente fortement.</p>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    step="0.1"
                ...
                {cbkValue > 0 && computedUnit > 0 && (
                  <p className="mt-2 text-center text-xs text-white/30">
                    → Avec votre BK actuelle : <span className="font-bold text-emerald-400">1U = {computedUnit.toFixed(2)}€</span> ({unitsInBk} unités)
                  </p>
                )}
              </div>
            )}
                  ...
                  {RECALC_OPTIONS.map((opt) => (
                    ...
                    >
                      ...
                      <p className={`mt-0.5 text-[9px] font-bold ${
                        autoRecalc === opt.value ? "text-emerald-400" : "text-white/30"
                      }`}>
                        {opt.label}
                      </p>
                    </button>
            ...
            {mode === "percent_bankroll" && (
              <div
                className="rounded-xl border border-emerald-500/20 p-3"
                style={{ background: "rgba(16,185,129,0.05)" }}
              >
                <p className="text-center text-xs text-emerald-400/70">
                  💡 En mode % de bankroll, votre mise est recalculée automatiquement après chaque résultat.
                </p>
                <p className="mt-1 text-center text-[10px] text-emerald-400/40">
                  Votre bankroll monte → votre mise monte. Elle descend → votre mise descend. C&apos;est la gestion la plus utilisée par les parieurs professionnels.
                </p>
              </div>
            )}
              ...
              >
                ...
                <div className="px-4 py-3">
                  <div className="flex items-center justify-between py-1.5">
                    <span className="text-xs text-white/40">Bankroll actuelle</span>
                  ...
                  <div className="flex items-center justify-between py-1.5">
                    ...
                    <span className="font-mono text-sm font-extrabold text-emerald-400">{computedUnit.toFixed(2)}€</span>
                  </div>
                  <div className="flex items-center justify-between py-1.5">
                  ...
                  <p className="mb-1.5 text-[9px] font-bold uppercase tracking-[0.15em] text-white/20">Quand le tipster publie un pick :</p>
                  {[0.5, 1, 2, 3].map((stake) => (
        ...
        {/* Reset to units only — only if currently in monetary mode */}
