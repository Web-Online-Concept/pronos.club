/**
 * PRONOS.CLUB — Format X pour le bilan mensuel (V3.5 Lot 11)
 *
 * Construit un thread X (5 posts) à partir d'un BilanMensuel.
 *
 * Réplique du pattern format-bilan-hebdo.ts adapté au mensuel.
 *
 * Structure :
 *   Post 1/5 : Header + KPIs principaux (ROI + profit + V/D/N + winrate)
 *   Post 2/5 : Stats par tier (lock/strong/value/coup_de_coeur)
 *   Post 3/5 : Top 4 sports les plus rentables
...
const buildHeaderPost = (bilan: BilanMensuel): string => {
...
const buildTierPost = (bilan: BilanMensuel): string => {
  const tiersOrder: Array<keyof BilanMensuel["picks_by_tier"]> = [
    "lock",
    "strong",
    "value",
    "coup_de_coeur",
  ...
  if (tiersWithPicks.length === 0) {
    lines.push("Aucune catégorie active ce mois.");
    return lines.join("\n");
  }
...
const buildSportPost = (bilan: BilanMensuel): string => {
  ...
  if (sportsArr.length === 0) {
    lines.push("Pas de données sport ce mois.");
    return lines.join("\n");
  }
...
const buildCLVPost = (bilan: BilanMensuel): string => {
  ...
  if (bilan.clv_avg_pct !== null && bilan.clv_picks_count > 0) {
    ...
    lines.push("⚪ Pas de données CLV ce mois.");
    lines.push("");
...
const buildLinkPost = (bilan: BilanMensuel, bilanLinkUrl: string): string => {
  const lines = [
...
/**
 * Construit le thread X complet pour un bilan mensuel.
 * Retourne un array de 5 textes prêts à passer à postThread().
 */
export const buildBilanMensuelThreadForX = (bilan: BilanMensuel): string[] => {
  const SITE_BASE_URL = process.env.NEXT_PUBLIC_SITE_URL ?? "https://pronos.club";
  const bilanLinkUrl = `${SITE_BASE_URL}/fr/pronos-ia/bilan-mensuel/${bilan.month_slug}?utm_source=x&utm_medium=social&utm_campaign=ia_bilan_mensuel_x`;
  ...
  return posts.map((post) => {
    if (post.length > X_MAX_TWEET_LENGTH) {
      console.warn(
        `[x-format-bilan-mensuel] Post dépasse ${X_MAX_TWEET_LENGTH} chars (${post.length}), troncature`
      );
